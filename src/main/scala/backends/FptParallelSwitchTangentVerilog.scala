  ...
  def emit(n: Int, r: Int, laneLog: Int, hw: ComplexHW[Double], scalingFactor: Complex[Double], inverse: Boolean = false, top: String = "main"): String =
    ...
    val blocksRtl = (0 until blocks).flatMap { block =>
      ...
      Vector(
        ...
        s"${top}Twist$block twist_$block(.clk(clk),.reset(reset),.next(pre_next_$block),.next_out(twist_next_$block),${Vector.tabulate(blockWidth)(lane => s".i$lane(pre_delay1_${block}_$lane)").mkString(",")},${connections("o", twistAt).mkString(",")});",
    ...
    val preDelayDeclarations = Vector.tabulate(blocks)(block => Vector.tabulate(blockWidth)(lane => s"reg [${width - 1}:0] pre_delay0_${block}_$lane,pre_delay1_${block}_$lane;")).flatten.mkString(" ")
    val preDelayLogic = Vector.tabulate(blocks) { block =>
      Vector.tabulate(blockWidth)(lane => s"pre_delay0_${block}_$lane<=pre_${block}_$lane;pre_delay1_${block}_$lane<=pre_delay0_${block}_$lane;")
    }.flatten.mkString(" ")
    s"""// Generated by SGen rate-preserving parallel switch FPT tangent ${if inverse then "inverse FFT" else "FFT"}.
       ...
       |  $inputTokenLogic
       |  $preDelayDeclarations
       |  $postDelayDeclarations
       |  always @(posedge clk) begin if(reset) begin ${if inverse then "" else ""} end else begin $preDelayLogic $postDelayLogic end end
       |  $blocksRtl
       |  $fftInstance
       |  $nextAssignment
       |  $outputAssignments
